...
export default function EditStudentPage() {
  ...
  const updateGuardian = (idx, key) => (e) =>
    setForm((s) => {
  ...
  const addGuardian = () =>
    setForm((s) => ({
      ...s,
      guardians: [...s.guardians, { full_name: "", phone: "", email: "" }],
    }));
  const removeGuardian = (idx) =>
    setForm((s) => ({
      ...s,
      guardians: s.guardians.filter((_, i) => i !== idx),
    }));
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const birthStr = data?.birth_date
    ...
    <main className="min-h-dvh p-6">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="mb-2 flex items-center justify-between">
          <Link
            href={`/controlPanel/students/${studentId}`}
