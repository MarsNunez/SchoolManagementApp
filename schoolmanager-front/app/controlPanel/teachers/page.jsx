...
export default function TeachersPage() {
  ...
  const resetForm = () => { setEditingId(""); setForm({ teacher_id: "", name: "", lastname: "", dni: "", specialties: "", email: "", phone: "", photo: "" }); };
  ...
  const submitItem = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await fetchJSON(`/teachers/${editingId}`, { method: "PUT", body: JSON.stringify(payload) });
      } else {
        await fetchJSON("/teachers", { method: "POST", body: JSON.stringify(payload) });
      }
      resetForm();
      await load();
    } catch (e) {
  ...
  const remove = async (teacher_id) => {
    ...
    try {
      await fetchJSON(`/teachers/${teacher_id}`, { method: "DELETE" });
      await load();
    } catch (e) {
      alert(e.message);
    }
    ...
    <main className="min-h-dvh p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        <header>
          ...
          <p className="text-sm text-neutral-500">List, create and manage teachers</p>
        </header>
        ...
        <section className="rounded-2xl border border-neutral-200/60 dark:border-neutral-800 bg-white/70 dark:bg-neutral-900/60 shadow-sm">
          <form onSubmit={submitItem} className="p-4 grid gap-3 sm:grid-cols-3">
            ...
            <input className="input" placeholder="specialties (comma separated)" value={form.specialties} onChange={(e)=>setForm({...form, specialties:e.target.value})} />
            <input className="input" placeholder="email" type="email" value={form.email} onChange={(e)=>setForm({...form, email:e.target.value})} />
            <input className="input" placeholder="phone" value={form.phone} onChange={(e)=>setForm({...form, phone:e.target.value})} />
            <input className="input sm:col-span-2" placeholder="photo url" value={form.photo} onChange={(e)=>setForm({...form, photo:e.target.value})} />
        ...
        <section className="rounded-2xl border border-neutral-200/60 dark:border-neutral-800 bg-white/70 dark:bg-neutral-900/60 shadow-sm overflow-x-auto">
          {loading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((t) => (
                  <tr key={t.teacher_id} className="border-b last:border-none border-neutral-100 dark:border-neutral-800">
                    <td className="p-3 whitespace-nowrap">{t.teacher_id}</td>
                    <td className="p-3 whitespace-nowrap">{t.name} {t.lastname}</td>
                    ...
                    <td className="p-3 flex gap-2">
                      ...
                      }} className="rounded-lg px-3 py-2 text-sm border border-neutral-300 dark:border-neutral-700">Edit</button>
                      <button onClick={() => remove(t.teacher_id)} className="btn-danger">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
const inputBase = "rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500";
const btnBase = "rounded-lg px-3 py-2 text-white text-sm font-medium transition-colors";
